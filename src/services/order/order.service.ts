...
export class OrderService {
    ...
    async add(cartId: number): Promise<Order | ApiResponse> {
        ...
        if (order) {
            return new ApiResponse("error", -7001, "An order for this cart has alredy been made.");
        }
        ...
        if (!cart) {
            return new ApiResponse("error", -7001, "No such cart found.");
        }
    ...
    async changeStatus(orderId: number, newStatus: "rejected" | "accepted" | "shipped" | "pending") {
        ...
        if (!order) {
            return new ApiResponse("error", -9001, "No such cart found!")
        }
